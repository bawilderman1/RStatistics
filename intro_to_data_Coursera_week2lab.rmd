...
sfo_feb_flights <- nycflights %>%
  filter(dest == "SFO", month == 2)
summarise(sfo_feb_flights,count = n())
...
<li> No flight is delayed more than 2 hours. ***Answer***</li> 
...
<li> Delta and United Airlines  ***Answer***</li>
...
nycflights %>% 
  group_by(month) %>% 
  summarise(mean_dd = mean(dep_delay)) %>% 
  summarise(maxMeanMonth = month[which(mean_dd == max(mean_dd))], maxAvg = max(mean_dd))
...
nycflights %>% 
  ...
  summarise(maxMedianMonth = month[which(median_dd == max(median_dd))], maxAvg = max(median_dd))
...
<li> Median would be more reliable as the distribution of delays is skewed. ***Answer***</li> 
...
nycflights %>%
  ...
  summarise(ot_dep_rate = sum(dep_type == "on time") / n()) %>%
  arrange(desc(ot_dep_rate)) %>% 
  summarise(maxOnTimeRate = origin[which(ot_dep_rate == max(ot_dep_rate))], maxOtDepRate = max(ot_dep_rate))
...
nycflights <- nycflights %>%
  mutate(air_time_hr = air_time /60, speed = distance/air_time_hr)
nycflights %>%
  group_by(tailnum) %>%
  summarise(avg_speed = max(speed)) %>% 
...
<li> There is an overall postive association between distance and average speed. ***Answer***</li> 
<li> There are no outliers. </li> 
<li> The distribution of distances are uniform over 0 to 5000 miles. </li> 
</ol>

```{r avg-speed-dist-scatter}
ggplot(nycflights, aes(x = distance, y = speed)) + geom_point()
